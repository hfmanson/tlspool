/* tlspool/tlsserver.c -- Take in a connection and run them as a command's streams 0 and 1 */
...
reconnect:
	...
	if ((cnx = accept (sox, NULL, 0))) {
		...
		if (do_pavlov) {
			if (pavlov (plainfd, plainfd, progname, pavlov_argc, pavlov_argv, timeout) != 0) {
				fprintf (stderr, "Pavlovian session failed on the server side\n");
				exit (1);
			}
		}
		//TODO// Consider starting another program, perhaps after a terminator for pavlov
